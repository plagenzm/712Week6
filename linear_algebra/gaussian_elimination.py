...
# MP note: modular function for validating data types for matrices. Use across different modules or functions that use linear algebra
# MP note: this function was generated by Copilot, but is more dynamic than the one we did in Commit #1 due to passing every arguement through the same variables
def validate_numeric_matrix(matrix, name="matrix"):
    try:
        arr = np.array(matrix, dtype=float64)
    except (TypeError, ValueError):
        raise TypeError("must be convertible to float64 NumPy array")
    if not np.issubdtype(arr.dtype, np.number):
        raise TypeError("must contain only numeric values.")
    return arr


# MP note: Commit #3 changed the error handling in this function to pass arguments through the validate_numeric_matrix function
def gaussian_elimination(coefficients, vector):
    #MP note: code generated by Copilot, approved by MP
    coefficients = validate_numeric_matrix(coefficients, "coefficients")
    vector = validate_numeric_matrix(vector, "vector")
